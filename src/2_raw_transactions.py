...
class RawTransactionsProcessor(BlockchainNodeAsAServiceConnector):
  ...
  def __init__(self, network, logger, api_key, akv_secret_name, vendor):
  ...
  def process_msg(self, tx_id):
    ...
    self.logger.info(f"API_request;{self.akv_secret_name}")
...
if __name__ == '__main__':
  ...
  raw_tx_processor = RawTransactionsProcessor(network, logger, api_key, secret_name, vendor)
  try:
    for msg in raw_tx_processor.consuming_topic(consumer_txs_hash):
      ...
      logger.info(f"mined_block_ingested;{tx_data['hash']};{tx_data['blockNumber']}")
