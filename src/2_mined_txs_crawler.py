...
if __name__ == '__main__':
  ...
  GROUP_ID = os.getenv("KAFKA_CG_TXS_IDS")
  ...
  TXS_PROCESSOR = RawTransactionsProcessor(NETWORK, LOGGER, AKV_CLIENT)
  ...
  PRODUCER_RAW_TX_DATA = SCHEMA_REGISTRY_HANDLER.create_avro_producer(
    producer_avro_configs,
    TXS_PROCESSOR.get_transaction_avro_schema()
  )
  ...
  for msg in TXS_PROCESSOR.consuming_topic(CONSUMER_HASH_TX_IDS):
    ...
    PRODUCER_RAW_TX_DATA.produce(topic, key=msg_key, value=cleaned_transaction_data, on_delivery=SCHEMA_REGISTRY_HANDLER.message_handler)
